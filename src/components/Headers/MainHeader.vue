...
const updateRoutes = () => {
    ...
    routesAccessMap.forEach((route, name) => {
        if (route?.hasAccess?.includes(role)) {
            if (route?.uses.has('top-menu') && route.mainMenuLabel) {
            ...
            if (route?.uses?.has('compact-menu')) {
                compactMenuRoutes.value.push({
                        name,
                        label: route.menuLabel || route.mainMenuLabel,
                        to: { name }
                    },
                )
            }
...
<template>
  <q-header class="the-header__wrapper">
    <div
      class="block-container layout-container full-height row no-wrap justify-between items-stretch"
    >
      ...
      <div class="the-header__profile">
        ...
        >
          ...
          <q-route-tab
            ...
            name="exit"
            :no-caps="true"
            :to="{ name: isLoggedIn ? 'mane-page' : 'auth-user-page' }"
            exact
            @click="logoutUser"
          >
