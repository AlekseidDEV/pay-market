...
const updateItem = async (rowIndex, newName) => {
  ...
  if (newName && typeof rowIndex === 'number') {
    ...
    const { error, data, success } = await $http.updateTargetAction(requestData)
    if (error) {
...
const addItem = async (newItemName) => {
  ...
  if (newItemName) {
    ...
    const { error, data, success } = await $http.createTargetAction(requestData)
    if (error) {
...
<template>
  <div class="settings-list-action-targets">
    <PmTable
      :rows="rowsData"
      :columns="columnsData"
      :hide-bottom="false"
  ...
  <SettingsAddListItem
    ...
    :title="currentRowIndex === null ? 'Добавить целевое действие' : 'Целевое действие'"
    ...
    <template #content>
      <div class="q-pa-lg">
        <div class="text-center">
          Вы действительно хотите удалить целевое действие
          <br>
